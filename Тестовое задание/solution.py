...
def gradient_check(X, y, weights, eps=1e-5):
    ...
    def check_parameters(num_gr, comp_gr, name):
        abs_diff = np.max(np.abs(num_gr - comp_gr))
        rel_diff = np.max(np.abs(num_gr - comp_gr) / (np.abs(comp_gr) + 1e-9))
